...
def main():
    try:
        ...
        MajaDataException, MajaDataMissingAngleException, MajaCotsException, MajaOTBCotsException, MajaOGRCotsException,
        MajaAlgorithmException, MajaChainException, MajaModuleException, MajaPluginBaseException, MajaPluginMuscateException,
        MajaPluginLandsat8Exception, MajaFactoryException, MajaProcessingException) as err:
        LOGGER.error(err)
        if LOGGER.level == logging.DEBUG:
            traceback.print_exc()
        LOGGER.error("Process NOK, exit code : "+str(err.get_exit_code()))
        sys.exit(err.get_exit_code())
