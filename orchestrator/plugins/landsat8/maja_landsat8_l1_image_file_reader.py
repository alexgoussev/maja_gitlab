...
class Landsat8L1ImageFileReader(L1ImageReaderBase):
    ...
    def generate_pix_mask(self, working_dir):
        ...
        for l1res in range(0,len(l_ListOfL1Resolution)):
            ...
            app = constant_image(dtm, 0, tmp_constant_filename, write_output=True)
    ...
    def generate_qb_mask(self, qb_filename, working_dir):
        ...
        app = OtbAppHandler("BinaryToVector", param_bintovec_qb, write_output=True)
    ...
    def generate_toa_sub_image(self, realL1Nodata,  working):
        ...
        for i, toa in enumerate(self._toascalarlist):
            ...
            img_toa_sub_bandmath = os.path.join(working, "aot_sub_bandmath_{}.tif".format(i))
            app_bandmath = band_math(
                [app_resample.getoutput().get("out"),self._edgsubmask],
                "im2b1>"+str(self._toathresholdminvalue)+"?"+str(realL1Nodata)+":im1b1",
                img_toa_sub_bandmath
                )
            self._toasublist.append(app_bandmath.getoutput().get("out"))

        self._toa_sub_pipeline.add_otb_app(app_bandmath)
        # end band loop
        ...
        self._subtoaimage = app_concatenate.getoutput().get("out")
        self._toa_sub_pipeline.add_otb_app(app_concatenate)

    def generate_l2_toa_images(self, l_ListOfL1Resolution, l_ListOfL2Resolution, working_dir):

        for l1res in range(len(l_ListOfL1Resolution)):
            ...
            l_l1bandidx = [self._plugin.BandsDefinitions.get_band_id_in_l2_coarse(b) for b in l_ListOfL1BandCode]
    ...
    def generate_edg_images(self, working_dir):
        ...
        for r in range(l_NbL2Res):
            ...
            app = resample(m_L2EDGThresholdImage, self._dem.ALTList[r], out_ressampling, OtbResampleType.LINEAR)
    ...
    def generate_l2_sat_images(self, l_ListOfL1Resolution, l_ListOfL2Resolution, working_dir):
        for l1res in range(len(l_ListOfL1Resolution)):
            curL1Res = l_ListOfL2Resolution[l1res]
            l_ListOfL1BandCode = self._plugin.BandsDefinitions.get_list_of_l1_band_code(curL1Res)
            l_ListOfL2BandCode = self._plugin.BandsDefinitions.get_list_of_l2_band_code(curL1Res)
            ...
            for band in l_ListOfL1BandCode:
                ...
                tmp_constant_sat_filename = os.path.join(working_dir, "SAT_constant_image_{}.tif".format(band))
            ...
            out_sat_concatenate = os.path.join(working_dir, "SATVectorImageList{}.tif".format(band))
            param_concatenate_sat = {"il": self._satmasklist,
                                     "out": out_sat_concatenate}
            concatenate_sat_app = OtbAppHandler("ConcatenateImages", param_concatenate_sat, write_output=True)
            self._sat_pipeline.add_otb_app(concatenate_sat_app)
            l_l1bandidx = [self._plugin.BandsDefinitions.get_band_id_in_l2_coarse(b) for b in l_ListOfL1BandCode]
            # L2SAT
            tmp_sat_roi = os.path.join(working_dir, "L2SATImageList_{}.tif".format(curL1Res))
            l2sat_roi_app = extract_roi(concatenate_sat_app.getoutput().get("out"),
                                        l_l1bandidx,
                                        tmp_sat_roi, write_output=True)
            self._sat_pipeline.add_otb_app(l2sat_roi_app)
            self._l2satmasklist.append(l2sat_roi_app.getoutput().get("out"))

    def generate_sat_sub_image(self, l2comm, working):
        dtm_coarse = self._dem.ALC
        # For each band of the input product
        l_satthresholdminvalue = l2comm.get_value("SaturationThresholdSub")
        l_satthresholdoutputvalue = 1
        l_satthresholdinsideputvalue = 0
        for i, sat in enumerate(self._satmasklist):
            # undersampling at L2CoarseResolution
            img_sat_sub = os.path.join(working, "sat_sub_{}.tif".format(i))
            app_resample = resample(sat, dtm_coarse, img_sat_sub, OtbResampleType.LINEAR_WITH_RADIUS,
                           write_output=True)
            self._sat_sub_pipeline.add_otb_app(app_resample)
            img_sat_sub_bandmath = os.path.join(working, "sat_sub_bandmath_{}.tif".format(i))
            app_bandmath = band_math(
                [app_resample.getoutput().get("out")],
                "im1b1>"+str(l_satthresholdminvalue)+"?"+
                str(l_satthresholdoutputvalue)+":"+str(l_satthresholdinsideputvalue),
                img_sat_sub_bandmath
                )
            self._satsublist.append(app_bandmath.getoutput().get("out"))

        self._sat_sub_pipeline.add_otb_app(app_bandmath)
        # end band loop
    ...
    def generate_cla_image(self, realL1Nodata, working_dir):
        LOGGER.debug("Cloud Altitude (CLA) image doesn't exist. Replaced by constant 0 image. !")
    ...
    def read(self, product_info, app_handler, l2comm, dem, pReadL1Mode):
        ...
        if pReadL1Mode == ReadL1Mode.READ_L1_MODE_FOR_ALGORITHMS:
            ...
            # ************************************************************************************************
            self.generate_l2_toa_images(l_ListOfL1Resolution, l_ListOfL2Resolution, working_dir)

            # ************************************************************************************************
            # L2SAT image pipeline connection
            # ************************************************************************************************
            self.generate_l2_sat_images(l_ListOfL1Resolution, l_ListOfL2Resolution, working_dir)
            ...
            # ************************************************************************************************
            self.generate_toa_sub_image(self._reall1nodata, working_dir)
            ...
            # ************************************************************************************************
            self.generate_cla_image(self._reall1nodata, working_dir)
